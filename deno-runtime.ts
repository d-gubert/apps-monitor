...
async function requestRouter({ type, payload }: Messenger.JsonRpcRequest): Promise<void> {
    ...
    if (method === 'app:setStatus' && args.spawnId > 0 && args.subprocess === '55ecd482-392f-447a-8fdc-a44b371ff794') {
        if (preventEnable && (params[0] === 'manually_enabled' || params[0] === 'auto_enabled')) {
            preventEnable = false;
            return Messenger.successResponse({ id, result: 'initialized' });
        }
    }
